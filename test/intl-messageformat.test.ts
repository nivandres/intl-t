...
import { injectVariables as iv } from "../src/tools";
...
describe("IntlMessageFormat", function () {
  ...
  it("should accept formatters", function () {
    ...
    expect(output).toMatch(/My name is Anthony Pipkin, age 8, time \d{1,2}:\d{1,2}:\d{1,2} [AP]M, date \d{1,2}\/\d{1,2}\/\d{2,4}\./);
  ...
  //       const mf = new IntlMessageFormat("");
  //       expect(mf.resolvedOptions().locale).toBe(new Intl.NumberFormat().resolvedOptions().locale);
  //     });
  ...
  describe("and changing the locale", function () {
    const simple = {
      ...
      fr: "{NAME} est {GENDER, select, " + "female {allée}" + "other {allé}}" + " à {CITY}.",
    };
    ...
    it("should format message en-US complex with different objects", function () {
      ...
      expect(msgFmt.format(maleTravelers)).toBe("Lucas, Tony and Drew went to Paris.");
    ...
    it("should format message fr-FR complex with different objects", function () {
      ...
      expect(msgFmt.format(maleTravelers)).toBe("Lucas, Tony and Drew sont allés à Paris.");
  ...
  describe("and change the locale with different counts", function () {
    const messages = {
      en: "{COMPANY_COUNT, plural, " + "=1 {One company}" + "other {# companies}}" + " published new books.",

      ru:
        "{COMPANY_COUNT, plural, " +
        "=1 {Одна компания опубликовала}" +
        "/1$/ {# компания опубликовала}" +
    ...
    it("should format a message with en-US locale", function () {
      ...
      expect(msgFmt.format({ COMPANY_COUNT: 0 })).toBe("0 companies published new books.");
      expect(msgFmt.format({ COMPANY_COUNT: 1 })).toBe("One company published new books.");
      expect(msgFmt.format({ COMPANY_COUNT: 2 })).toBe("2 companies published new books.");
      expect(msgFmt.format({ COMPANY_COUNT: 5 })).toBe("5 companies published new books.");
      expect(msgFmt.format({ COMPANY_COUNT: 10 })).toBe("10 companies published new books.");
    });

    it("should format a message with ru-RU locale", function () {
      const msgFmt = new IntlMessageFormat(messages.ru, "ru-RU");

      expect(msgFmt.format({ COMPANY_COUNT: 0 })).toBe("0 компаний опубликовали новые книги.");
      expect(msgFmt.format({ COMPANY_COUNT: 1 })).toBe("Одна компания опубликовала новые книги.");
      expect(msgFmt.format({ COMPANY_COUNT: 2 })).toBe("2 компании опубликовали новые книги.");
      expect(msgFmt.format({ COMPANY_COUNT: 5 })).toBe("5 компаний опубликовали новые книги.");
      expect(msgFmt.format({ COMPANY_COUNT: 10 })).toBe("10 компаний опубликовали новые книги.");
      expect(msgFmt.format({ COMPANY_COUNT: 21 })).toBe("21 компания опубликовала новые книги.");
    });
  });
  ...
  describe("arguments with", function () {
    ...
    describe("a numeral", function () {
      ...
      //   expect(formatWithMissingValue).toThrow(new MissingValueError("ST1ATE", "{ST1ATE}"));
      // });
  ...
  describe("selectordinal arguments", function () {
    const msg = "This is my {year, selectordinal, number, one{#st} two{#nd} three{#rd} other{#th}} birthday.";
  ...
  describe("exceptions", function () {
    ...
    it("should take negative number as plural", function () {
      const msg = "{num, plural, offset:-1 =-1{negative one} =0{one} other{other}}";
      const mf = new IntlMessageFormat(msg, "en");
    ...
    it("should take empty string value", function () {
      ...
      // expect(mf.formatToParts({ value: "" })).toEqual([{ type: PART_TYPE.literal, value: '""' }]);
  ...
  it("should handle offset in plural #", function () {
    ...
    const mf = new IntlMessageFormat(msg, "en");
    expect(mf.format({ host: "The host", guest: "Alice", num_guests: 0 })).toBe("The host does not give a party.");
    expect(mf.format({ host: "The host", guest: "Alice", num_guests: 1 })).toBe("The host invites Alice to their party.");
  ...
  it("regression issue #437", function () {
    const mf = new IntlMessageFormat("{score, plural, one {# shopper} other {# shoppers}}", "en");
  ...
  //     expect(mf.format<object>({ b: parts => ({ parts }) })).toEqual(["hello ", { parts: ["world"] }]);
  //   });
  //   it("nested tag message", function () {
  //     const mf = new IntlMessageFormat("hello <b>world<i>!</i> <br/> </b>", "en");
  //     expect(
  //       mf.format<object>({
  //         b: chunks => ({ chunks }),
  //         i: c => ({ val: `$$${c}$$` }),
  ...
  //     const mf = new IntlMessageFormat("hello <b>world</b> <a>{placeholder}</a>", "en");
  //     expect(
  //       mf.format<object>({
  //         b: parts => ({ parts }),
  //         placeholder: "gaga",
  ...
  //     const mf = new IntlMessageFormat("&lt; hello <b>world</b> {token} &lt;&gt; <a>{placeholder}</a>", "en");
  //     expect(
  //       mf.format<object>({
  //         b: parts => ({ parts }),
  //         token: "<asd>",
  ...
  //         a: parts => ({ parts }),
  //       }),
  //     ).toEqual(["&lt; hello ", { parts: ["world"] }, " <asd> &lt;&gt; ", { parts: [">"] }]);
  //   });
  ...
  //         a: str => ({ str }),
  //       }),
  //     ).toEqual(["&lt; hello ", { str: ["world"] }, " <asd> &lt;&gt; ", { str: [">"] }]);
  //     expect(
  //       mf.format<object>({
  //         gender: "female",
  //         b: str => ({ str }),
  ...
  describe("# symbol in plural rule argument", () => {
    it("replaces unquoted # symbol in the plural rule option with actual number", () => {
      const mf = new IntlMessageFormat("You {count, plural, one {worked for # hour} other {worked for # hours}} today.", "en");
    ...
    it("~does not~ replace # symbol in deeply nested sub messages", () => {
      ...
      expect(mf.format({ count: 1, unit: "hour" })).toBe("You worked for 1 hour today.");
      expect(mf.format({ count: 3, unit: "hour" })).toBe("You worked for 3 hours today.");
    });
  });
  ...
  it("number skeleton", function () {
    // expect(new IntlMessageFormat("{amount, number, ::currency/CAD .0 group-off}", "en-US").format({ amount: 123456.78 })).toMatch(
    //   /\$123456.8/,
    // ); // Deal w/ IE11
    expect(new IntlMessageFormat("{amount, number, ::currency/GBP .0#}", "en-US").format({ amount: 123456.789 })).toBe("£123,456.79");
  });

  it("number skeleton with scale", function () {
    expect(new IntlMessageFormat("{amount, number, ::percent scale/0.01}", "en-US").format({ amount: 12.3 })).toBe("12%");
  });
  ...
  //     const element = {};
  //     const parts = new IntlMessageFormat("a react {element}", "en").formatToParts({
  //       element,
  //     });
  ...
  describe("no locale", function () {
    describe("no locale provided", function () {
      it("should default to English", function () {
        const msg = new IntlMessageFormat("I have {NUM_BOOKS, plural, =1 {1 book} other {# books}}.");
        // expect(msg.resolvedOptions().locale).toBe(new Intl.NumberFormat().resolvedOptions().locale);
    ...
    describe("invalid locale default", function () {
      it("should fallback to default locale", function () {
        const msg = new IntlMessageFormat("{COMPANY_COUNT, plural, =1 {One company} other {# companies}} published new books.", "fu-BA");
