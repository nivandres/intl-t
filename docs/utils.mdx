...
Intl-t provides a set of tools to help you with your translations. You can use each of them independently from `intl-t/utils` or `@intl-t/utils`.
...
```ts
import { inject } from "intl-t/utils";
...
```ts
import { negotiator } from "intl-t/utils";
...
```ts
import { format } from "intl-t/utils";
```
...
Resolver functions are best used via [createNavigation](/docs/next#navigation), but you can also import them directly from `intl-t/utils` without bound values and types.
