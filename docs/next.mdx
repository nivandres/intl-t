...
- **`pathPrefix`**: Controls how the locale appears in the URL path.
  ...
  - `"hidden"`: The locale is never shown in the path prefix.  
    _Default is `"default"` for `strategy: "param"` and `"hidden"` for `strategy: "request"`._
...
- **`pathBase`**: Determines the behavior when no locale is specified in the path.
  ...
  - `"detect-latest"`: On the first visit, the user's locale is detected and redirected; subsequent visits at path base go to the most recently used locale.  
    _Default is `"detect-default"` when the effective `pathPrefix` is visible and `"detect-latest"` when it is `"hidden"`._

- **`strategy`**: Specifies how to match the locale and path.
  - `"param"`: Uses the locale path segment, typically the Next.js `[locale]` param. This is the static-friendly mode.
  - `"request"`: Resolves the locale dynamically from request state, such as custom detection, headers, cookies, or domain-derived logic. Middleware stores the resolved locale in both headers and cookies for later reads.

  `"request"` does not require a `[locale]` route segment. Its default canonical URL is unprefixed, but you can still expose locale prefixes by setting `pathPrefix`.
...
```tsx
createNavigation({ allowedLocales, strategy: "request" });
```
