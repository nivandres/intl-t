...
describe("inject — replacement safety", () => {
  it("keeps $-patterns in variable values literal", () => {
    expect(injectVariables("{v}", { v: "$&" } as any)).toBe("$&");
    expect(injectVariables("pre {v} post", { v: "$'" } as any)).toBe("pre $' post");
  });
});
...
describe("inject — plural/select matrix (volume)", () => {
  it("resolves exact and category plural branches", () => {
    ...
    expect(injectVariables(msg, { n: 0 } as any)).toBe("none");
    expect(injectVariables(msg, { n: 1 } as any)).toBe("one item");
    expect(injectVariables(msg, { n: 7 } as any)).toBe("7 items");
  });

  it("resolves select with fallback to other", () => {
    const msg = "{g, select, male {él} female {ella} other {elle}}" as string;
    expect(injectVariables(msg, { g: "female" } as any)).toBe("ella");
    expect(injectVariables(msg, { g: "unknown" } as any)).toBe("elle");
  });

  it("interpolates sibling variables inside a matched option", () => {
    const msg = "{n, plural, other {# messages for {name}}}" as string;
    expect(injectVariables(msg, { n: 2, name: "Ivan" } as any)).toBe("2 messages for Ivan");
  });

  it("handles multiple independent placeholders in one string", () => {
    expect(injectVariables("{a}-{b}-{c}" as string, { a: 1, b: 2, c: 3 } as any)).toBe("1-2-3");
  });

  it("keeps siblings interpolated when a plural variable is missing (continue semantics)", () => {
    const out = injectVariables("{n, plural, other {# items}} & {name}" as string, { name: "ok" } as any);
...
describe("inject — escaped braces", () => {
  it("unescapes every escaped brace, not only the first", () => {
    ...
    const out = injectVariables("`{a`} and `{b`}" as string, {} as any);
...
describe("inject — falsy and nullish variable values", () => {
  it("injects 0, '' and false literally instead of dropping them", () => {
    expect(injectVariables("n={n}", { n: 0 } as any)).toBe("n=0");
    expect(injectVariables("s=[{s}]", { s: "" } as any)).toBe("s=[]");
    expect(injectVariables("b={b}", { b: false } as any)).toBe("b=false");
  });

  it("stringifies null but leaves the placeholder for undefined", () => {
    expect(injectVariables("v={v}", { v: null } as any)).toBe("v=null");
    expect(injectVariables("v={v}", { v: undefined } as any)).toBe("v={v}");
  });
});

describe("inject — emoji and brace-bearing content", () => {
  it("injects emoji values", () => {
    expect(injectVariables("go {v}!", { v: "🚀" } as any)).toBe("go 🚀!");
  });

  it("does not match emoji placeholder NAMES (word-character names only, frozen limitation)", () => {
    expect(injectVariables("go {🚀}!", { "🚀": "up" } as any)).toBe("go {🚀}!");
  });

  it("keeps braces from a value literal when no matching variable exists, but cascades when one does", () => {
    expect(injectVariables("v={v}", { v: "{x}" } as any)).toBe("v={x}");
    ...
    expect(injectVariables("v={v}", { v: "{x}", x: "cascaded" } as any)).toBe("v=cascaded");
  });
});
...
describe("inject — purity and scale", () => {
  it("stays correct across 1000 sequential injections with distinct values", () => {
    for (let i = 0; i < 1000; i++) {
      expect(injectVariables("hi {name} #{i}", { name: "u" + i, i } as any)).toBe(`hi u${i} #${i}`);
    }
  });

  it("replaces 10k occurrences of the same placeholder in one ~80KB template", () => {
    const template = "word {v} ".repeat(10_000);
    const out = injectVariables(template, { v: "X" } as any);
    expect(out.includes("{v}")).toBe(false);
    expect(out).toBe("word X ".repeat(10_000));
  });

  it("resolves ICU plural branches at numeric extremes", () => {
    expect(injectVariables("{n, plural, =0 {none} one {one} other {# items}}" as string, { n: 0 } as any)).toBe("none");
    expect(injectVariables("{n, plural, one {one} other {# items}}" as string, { n: 1000 } as any)).toBe("1000 items");
  });
});

describe("inject — onMissingVariable policies", () => {
  it("keeps the placeholder by default", () => {
    expect(injectVariables("hi {name}", {} as any)).toBe("hi {name}");
  });

  it("clears the placeholder with the empty policy", () => {
    expect(injectVariables("hi {name}!", {} as any, { onMissingVariable: "empty" } as any)).toBe("hi !");
  });

  it("uses the function policy's return as the value", () => {
    expect(injectVariables("hi {name}", {} as any, { onMissingVariable: (k: string) => `[${k}?]` } as any)).toBe("hi [name?]");
  });
...
describe("inject — numeric strings stay literal (plain interpolation)", () => {
  it('preserves "007" and hex-like strings', () => {
    expect(injectVariables("code {v}", { v: "007" } as any)).toBe("code 007");
    expect(injectVariables("id {v}", { v: "0x1F" } as any)).toBe("id 0x1F");
  });
});
...
describe("inject — date/time heuristic matrix (supported surface under control)", () => {
  ...
  const ctx = { locale: "en-US" } as any;

  it("formats explicit date styles (short/md/long/full)", () => {
    expect(injectVariables("{d, date, short}" as string, { d } as any, ctx)).toBe(en({ dateStyle: "short" }));
    expect(injectVariables("{d, date, md}" as string, { d } as any, ctx)).toBe(en({ dateStyle: "medium" }));
    expect(injectVariables("{d, date, long}" as string, { d } as any, ctx)).toBe(en({ dateStyle: "long" }));
    expect(injectVariables("{d, date, xl}" as string, { d } as any, ctx)).toBe(en({ dateStyle: "full" }));
  });

  it("routes time-named tokens to timeStyle", () => {
    expect(injectVariables("{d, time}" as string, { d } as any, ctx)).toBe(en({ timeStyle: "short" }));
  });

  it("verbose combines date and time styles", () => {
    expect(injectVariables("{d, date, verbose}" as string, { d } as any, ctx)).toBe(en({ dateStyle: "long", timeStyle: "long" })); // verbose matchea el grupo long
  });

  it("keeps two date tokens isolated (no style leak between placeholders)", () => {
    const out = injectVariables("{a, time} | {b, date, short}" as string, { a: d, b: d } as any, ctx);
