...
const localeDescriptor = Object.getOwnPropertyDescriptor(state, "locale");

afterEach(() => {
  if (localeDescriptor) Object.defineProperty(state, "locale", localeDescriptor);
});
...
describe("resolvers", () => {
  describe("resolveLocale", () => {
    ...
    it("supports direct allowed locales and rejects invalid candidates", () => {
      expect(resolveLocale("/es/docs", ["en", "es"])).toBe("es");
      expect(resolveLocale("../../etc/passwd", ["en", "es"])).toBeUndefined();
    });
  ...
  describe("resolveHref", () => {
    ...
    it("uses current locale and redirect path fallbacks when no explicit locale is provided", () => {
      expect(resolveHref("/docs", { currentLocale: "es" })).toBe("/es/docs");
      expect(resolveHref("/docs", { currentLocale: undefined, getLocale: () => undefined, redirectPath: "r" })).toBe("/r/docs");
    });

    it("uses the default locale resolver and supports hidden prefixes", () => {
      Object.defineProperty(state, "locale", {
        configurable: true,
        value: "es-CO",
      });

      expect(resolveHref("/docs", { allowedLocales: ["en", "es"], defaultLocale: "en" })).toBe("/es/docs");
      expect(resolveHref("/docs", { allowedLocales: ["en", "es"], pathPrefix: "hidden" })).toBe("/docs");
    });
  });
  ...
  describe("resolvePath", () => {
    ...
    it("does not strip unrelated leading segments that only look like locale prefixes", () => {
      expect(resolvePath("/enough/docs", ["en", "es"])).toBe("/enough/docs");
    });
